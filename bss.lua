-- NO LONGER MAINTENCE DUE TO OWNER GOT FILE RESET (me)
repeat task.wait(0.1) until game:IsLoaded()

--getgenv().LoadRecent = false -- remove the 2 first "--" if you want to use this to prevent from disconnecting
--getgenv().AutoLoad = "" -- type your config name here
getgenv().Star = "⭐"
getgenv().Danger = "⚠️"
...
local library = loadstring(game:HttpGet("https://pastebin.com/raw/SV5PzfGM"))()
getgenv().api = loadstring(game:HttpGet("https://pastebin.com/raw/KuuW0ZGv"))()
local bssapi = loadstring(game:HttpGet("https://pastebin.com/raw/R6u0EueK"))()
...
local function useConvertors()
    ...
    if lastWithoutCooldown ~= nil and 
    string.find(kocmoc.vars.autouseMode, "Honey Wreath") or 
    string.find(kocmoc.vars.autouseMode, "All") then
        if converted == false then
            game:GetService("ReplicatedStorage").Events.ToyEvent:FireServer(lastWithoutCooldown)
    ...
    if GetItemListWithValue()["Stinger"] > 0 and 
    string.find(kocmoc.vars.autouseMode, "Stinger") or 
    string.find(kocmoc.vars.autouseMode, "All") then
...
local Config = {
    WindowName = "BssThePoggest v" .. temptable.version .. " Remastered By Ur Mom",
...
guiElements["vars"]["autouseMode"] = contt:CreateDropdown("Bag Reduction Mode", {
    "Ticket Converters", "Just Snowflakes", "Just Coconuts", "Just Wreath", "Just Stinger",
    "Coconuts and Snowflakes", "Snowflakes and Tickets", "Coconuts and Tickets",
    "Wreath and Tickets", "Coconuts and Wreath", "Snowflakes and Wreath",
    "Coconut and Stinger", "Snowflakes and Stinger", "Wreath and Stinger", "Tickets and Stinger",
    "All"
...
guiElements["toggles"]["trainsnowbear"] = mobkill:CreateToggle("Train Snow Bear", nil, function(State) kocmoc.toggles.trainsnow = State end)
                            ...
                            if (kocmoc.toggles.buckobeequests and questName:find("Bucko Bee")) or (kocmoc.toggles.rileybeequests and questName:find("Riley Bee")) or (kocmoc.toggles.polarbearquests and questName:find("Polar Bear")) or (kocmoc.toggles.brownbearquests and questName:find("Brown Bear")) or (kocmoc.toggles.blackbearquests and questName:find("Black Bear")) or (kocmoc.toggles.allquests and not questName:find("Bear:") and not questName:find("Bee:")) then
                                if not string.find(v.Text, "Puffshroom") then
                                    if text:find(" Goo ") then
                                        ...
                                        if text:find (" Goo " .. " Comepleted!") then
                                            temptable.usegumdropsforquest = false
                                        end
                                    end
                ...
                if tonumber(temptable.pollenpercentage) < tonumber(kocmoc.vars.convertat) or kocmoc.toggles.disableconversion and not temptable.planting then
                    if not temptable.tokensfarm then
                        ...
                        if kocmoc.toggles.killmondo then
                            while kocmoc.toggles.killmondo and game.Workspace.Monsters:FindFirstChild("Mondo Chick (Lvl 8)") and not temptable.started.vicious and not temptable.started.monsters and not temptable.started.windy do
...
if getgenv().LoadRecent and isfile('kocmoc/recent.json') then
